(function($) {
	$(document).ready(function() {
		...
		function gardenia_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					gardenia_remove_file( $(this).parents('.section') );
				});
			});
		...
		function gardenia_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				gardenia_add_file(event, $(this).parents('.section'));
			});
		}
